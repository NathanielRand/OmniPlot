<script lang="ts">
	...
	import { PPF_ZONES_LIST, TINT_ZONES_LIST } from "$lib/stores/patternStore.svelte";
	...
	// When category changes, reset zone to first option
	$effect(() => {
		const first = zoneList[0];
		if (first && !zoneList.find(z => z.value === pattern.zone)) {
			pattern.zone = first.value;
		}
	});

	function onZoneChange(e: Event) {
		pattern.zone = (e.target as HTMLSelectElement).value as PatternZone;
	}
	...
	onMount(async () => {
		...
		try {
			...
			original = p;
			vehicle  = { make: p.make, model: p.model, year: p.year, bodyStyle: p.bodyStyle };
			pattern  = {
				category:     p.category,
				zone:         p.zone,
				name:         p.name,
	...
	async function handleSave(e: SubmitEvent) {
		...
		try {
			...
			toastStore.success("Pattern saved", `${pattern.name.trim()} has been updated.`);
			goto("/library?tab=mine");
		} catch {
			...
			<form class="edit-form" onsubmit={handleSave} novalidate>
				...
				<section class="form-section">
					...
					<div class="field-row field-row--3">
						<div class="field" class:field--error={errors.year}>
							...
							<input id="year" class="field__input" type="number" min="1950"
								max={new Date().getFullYear() + 2} bind:value={vehicle.year}/>
						...
						<div class="field" class:field--error={errors.make}>
							...
							<input id="make" class="field__input" type="text" bind:value={vehicle.make} autocomplete="off"/>
						...
						<div class="field" class:field--error={errors.model}>
							...
							<input id="model" class="field__input" type="text" bind:value={vehicle.model} autocomplete="off"/>
				...
				<section class="form-section">
					...
					<div class="field" class:field--error={errors.name}>
						...
						<input id="patternName" class="field__input" type="text" bind:value={pattern.name}/>
					...
					<div class="field" class:field--error={errors.svgPath}>
						<label class="field__label" for="svgPath">SVG Path Data</label>
